...
#Forced Burger's Equation, Matern kernel
...
coef=1 #coefficient of the double x derivative term in the heat equation
pi=np.pi


def Burgerforced(N,M):
    ...
    #below are sigma and the derivatives wrt x and t individually
    ...
    def F(t, y, x): return 10*np.sin(2*pi*x*omega)*np.cos(t*pi/10)+2*np.abs(np.sin(x*pi*omega)*np.cos(t*pi/10))
    ...
    truesolution=g_x
    truegradient = F(0, 1, x)
    for i in range(1,N):
        truesolution = np.append(truesolution, matlabsubset[:,i])
        truegradient = np.append(truegradient, F(t[i], 1, x))
    ...
    import csv
    with open("Burgerforcedresults.txt", "a") as f:
        wr = csv.writer(f)
        wr.writerow(results)
        f.close()
