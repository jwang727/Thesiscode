...
def Porous(N,M):
    ...
    def dr1r1x_d2_sigma(t1,t2,r1,r2,uhat2,uhat1grad):
        return np.kron(dr2_k(t1,t2),dr1r1_c(r1,r2))-coef*np.kron(k(t1,t2),dr1r1r2r2_c(r1, r2))*uhat2-coef*np.kron(k(t1,t2),dr1r1r2_c(r1, r2))*uhat1grad
    ...
    # mean of u(t_{0},x), du(t_{0},x)/dx from the current prior mean to linearise the differential operator at the next timepoint t_0
    ...
    for i in range(1,stop+1):
        ...
        d2_sigma_block = np.column_stack((sigma(t,t_0,x,x[1:-1]), sigma(t,t_i,x,x0),sigma(t,t_i,x,xL), d2_sigma(t, t_i, x, x,1,uhatmat,uhatmatgrad)))

        d1_d2_sigma_block = np.block([
            [sigma(t_0,t_0,x[1:-1],x[1:-1]), sigma(t_0,t_i,x[1:-1],x0),sigma(t_0,t_i,x[1:-1],xL),d2_sigma(t_0, t_i, x[1:-1],x,1,uhatmat0,uhatmat0grad)],
            [sigma(t_i,t_0,x0,x[1:-1]),sigma(t_i,t_i,x0,x0),sigma(t_i,t_i,x0,xL),d2_sigma(t_i, t_i, x0,x,1,uhatmatix0,uhatmatix0grad)],
            [sigma(t_i,t_0,xL,x[1:-1]), sigma(t_i, t_i, xL, x0), sigma(t_i, t_i, xL, xL), d2_sigma(t_i, t_i, xL, x, 1, uhatmatix0,uhatmatix0grad)],
            [d1_sigma(t_i, t_0, x,x[1:-1],1,uhatmat0.T,uhatmat0grad.T),d1_sigma(t_i, t_i, x,x0,1,uhatmatix0.T,uhatmatix0grad.T),d1_sigma(t_i, t_i, x,xL,1,uhatmatix0.T,uhatmatix0grad.T),d1_d2_sigma(t_i, t_i, x,x,1,uhatmati,uhatmati.T,uhatmatigrad,uhatmatigrad.T)]
        ])


        d1_d2_sigma_block_inverse=np.linalg.inv(d1_d2_sigma_block)

        dr1x_d2_sigma_block_left = np.column_stack((dr1x_sigma(t, t_0, x, x[1:-1]), dr1x_sigma(t, t_i, x, x0),dr1x_sigma(t, t_i, x, xL), dr1x_d2_sigma(t, t_i, x, x, uhatmat,uhatmatgrad)))
        ...
        d1_d2_sigma_block_left = np.column_stack((d1_sigma(t, t_0, x, x[1:-1], 1, uhatmat0allt.T,uhatmat0alltgrad.T), d1_sigma(t, t_i, x, x0, 1, uhatmatix0allt.T,uhatmatix0alltgrad.T),d1_sigma(t, t_i, x, xL, 1, uhatmatix0allt.T,uhatmatix0alltgrad.T), d1_d2_sigma(t, t_i, x, x, 1,uhatmat,uhatmatiallt.T,uhatmatgrad,uhatmatialltgrad.T)))
    ...
    import csv
    with open("Porousresults.txt", "a") as f:
        wr = csv.writer(f)
        wr.writerow(results)
    f.close()
